...
- Codex 完了後、受理条件を機械的に検証する:
  ...
  2. `git diff origin/main --stat -- src/` が空であること(プロダクトコード無変更)
...
Working Style)。修正するか・Issue 化するか・監査 PR をどう扱うかはオーナー判断。

オーナーが修正する場合、監査ブランチに修正コミットを足せば red → green となり、失敗テストがそのまま
regression test(CONTRIBUTING.md のテスト観点)としてマージできる。
