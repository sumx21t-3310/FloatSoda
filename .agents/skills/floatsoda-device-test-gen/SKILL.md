...
### 6. カタログサンプルの目視チェック項目を生成する

対象は `samples/FloatSoda.Samples.*` のカタログサンプル(ハーネスシナリオとは別物)。各サンプルの
Demo.cs を読み、目視で区別できるデモケース(色分け・比較ペア・プロパティ違いのセルなど)を漏れなく
抽出し、`samples/<Name>/checks/items.jsonl` に書き出す。

**items.jsonl のスキーマ**(1行1項目、JSON Lines):
```json
...
- `flutter_ref`: 対応する Flutter 本家実装の `<ファイル>:<行>`(該当する概念クラスの場所)。
  分からなければ空文字。
- `question`: 何を確認する項目かの説明文(見るべき点の説明)。
- `read_aloud`: オーナーが HMD を被ったまま聞いて yes/no で答えられる短い口頭指示
  (20〜40文字程度)。

この手順は、新規サンプル追加時にも、既存サンプルが変わった(Demo.cs の行番号がずれた、ケースが
増減した)ときの再生成にも同じ形で使う。生成後は必ず `source` の `file:line` を実際に読んで
検証すること — ハルシネーション対策は上の「1. 列挙する」節と同じ姿勢。
...
カタログサンプルの目視チェック項目(手順6)を生成した場合は、何サンプル分の items.jsonl を
生成/更新したかを報告に含める。
