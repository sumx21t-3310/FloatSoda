...
- **easy** — getting-started の手順がそもそも機能するか(オーバーレイが1枚出るか)
...
| ⓒ | **ライブラリバグ** | API は実在し docs どおりに使われたが、ライブラリが壊れている | 実装を読み、欠陥と、それを起こす呼び出しの流れを確認する(例: `CleanChildRelayoutBoundary` が `this` に再帰) |
