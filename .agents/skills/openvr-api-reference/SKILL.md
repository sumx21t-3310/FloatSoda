...
- **既存のラップ方針との整合性**: `src/FloatSoda.OVR/` に既にある型(`OVRApplication.cs` など)
  ...
  (例: `143-ovr-applications-launch-control`)が示す現在の作業スコープと関係があるかも確認する。
- **AGENTS.mdの原則**: 状態を増やす場合、コードで表現できない外部ファイル/設定に依存しないか
  (uGUI回避エンジニア向けの価値提案)。
...
このスキル自体はコードを変更しない。AGENTS.mdの「デフォルトでは調査・説明のみ、実装はオーナーの
