...
このゲートは**最終的にタグを打つコミットに対して有効である必要があります**。ゲートを通したコミットの SHA を控えておき、手順7で照合します。
...
**タグ対象のコミットがジュニアコーダーゲートの検証範囲に収まっていることを確認します。** ゲートは特定のコミットの docs と public API に対する検証です。手順5でゲートを通したのはリリースコミットを作る前の状態なので、`RELEASE_SHA` そのものはゲートを通っていません。

ゲートが測っている面(docs と public API)が変わっていなければ再実行は不要です。それを差分で確認します。

```bash
GATED_SHA=<手順5でゲートを通したコミットのSHA>
