...
<!-- 派生互換スタブ — 直接編集禁止。正典: .agents/skills/floatsoda-device-test-run/ -->

**このスキルの正典は [`.agents/skills/floatsoda-device-test-run/SKILL.md`](../../../.agents/skills/floatsoda-device-test-run/SKILL.md)。そのファイルを読み、そこにある手順に従うこと。** `references/` ファイルは正典の隣、`.agents/skills/floatsoda-device-test-run/references/` にある。
