...
| 処理を直列化するための専用スレッド | `Channel<T>`(単一リーダー) | 立てない。専用スレッドはスレッド親和性が要求される場合（GL コンテキスト等）に限る |

最後の行で `Channel<T>`(単一リーダー)だけを挙げているのは、**「投入順に1件ずつ」まで保証する標準機構が他にないため**です。似て見えるものは、保証している契約が違います。

| 機構 | 保証すること | 保証しないこと |
|---|---|---|
| `Channel<T>`(単一リーダー) | 排他実行と投入順 | — |
