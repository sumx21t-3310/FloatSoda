...
/// <summary>
/// FizzyPop デザインシステムのボタン。振る舞いは <see cref="ButtonBase"/> に委譲し、
...
public record Button : StatelessWidget
{
    ...
    public override Widget Build(IBuildContext context)
    {
        return new ButtonBase
        {
            ...
            Builder = (ctx, state) =>
            {
                var style = Style ?? FizzyPopTheme.Of(ctx)?.ButtonStyle ?? ButtonStyle.Default;
                ...
                // TODO: BackdropFilter相当のレンダー機能実装後に背景ブラーを適用する
                // (現状は半透明ベタ塗りまで。SKImageFilter.CreateBlur ベースのレイヤーが必要)
