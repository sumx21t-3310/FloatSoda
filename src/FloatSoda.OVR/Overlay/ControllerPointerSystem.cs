...
/// <summary>
/// <see cref="VROverlayFlags.MakeOverlaysInteractiveIfVisible"/> を使わずに、コントローラーレイとオーバーレイの
/// 交差を毎フレーム手動で計算しポインターイベントを生成します。
/// SteamVR の入力横取りが発生しないため、VRChat の AFK 判定を回避できます。
/// </summary>
/// <param name="intersection">The overlay intersection service used for ray casting.</param>
public sealed class ControllerPointerSystem(OverlayIntersection intersection)
{
    ...
    /// </summary>
    /// <param name="device">The logical controller to poll.</param>
    /// <param name="trackingOrigin">The tracking universe used for the pose and intersection ray.</param>
    /// <returns>Pointer data for a hit, or <see langword="null"/> when the device is unavailable or the ray misses.</returns>
