...
- **README に架空のコードを書かない。** コードブロックは `<Name>Demo.cs` に実在する記述の抜粋にします。読みやすさのために `Child =` のような外側の文脈を落とすのは構いませんが、**Demo に無い API 呼び出しやプロパティを書かないでください**。README とコードの乖離を構造的に防ぐためです。
